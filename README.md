...
> **⚠ より重要な影響: 未信頼のパッケージは `brew outdated` からも静かに消える**
>
> 実測（2026-08-03）では、`kayac/tap/ecspresso` が未信頼の間、全パッケージを更新した後の
> `brew outdated` は**空**を返していた。信頼を宣言した直後に同じコマンドを実行すると
> `ecspresso (2.8.4) < 2.8.5` が現れた。
>
> つまり未信頼の間、このパッケージは鮮度チェックの網から外れており、
> **「全て最新」という表示自体が嘘になっていた**。更新漏れがセキュリティ修正だった場合、
> 気づく手段が無くなる。
>
> 新しい tap を追加したら、その場で必要な formula の信頼を宣言すること。
> 「警告が出ているが実害はなさそう」で放置すると、監視の穴になる。
